...
class kryptono extends Exchange {
    ...
    public function fetch_markets ($params = array ()) {
        $response = $this->v2GetMarketPrice ($params);
        //
        //     array (
        //         array (
        //             "$symbol":"LTC-BTC",
        ...
        for ($i = 0; $i < count ($response); $i++) {
            $market = $response[$i];
            $name = $this->safe_string($market, 'symbol');
            $baseId = explode('_', $name)[1];
            $quoteId = explode('_', $name)[0];
    ...
    public function fetch_currencies ($params = array ()) {
        ...
        for ($i = 0; $i < count ($currencies); $i++) {
            ...
            $id = $this->safe_string($currency, 'currency_code');
        ...
        if (is_array($response) && array_key_exists('history', $response)) {
            if ($response['history'] !== null) {
                $history = $response->history.map ((item) => {
                    return array( ...item, 'timestamp' => item.time );
                });
